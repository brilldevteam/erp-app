...
export default function BrandSettings() {
    ...
    // Default logo paths
    const defaultLogos = {
        logo_dark: getImagePath('/packages/workdo/Appointment/src/Resources/images/logo.png'),
        favicon: getImagePath('/packages/workdo/Appointment/src/Resources/images/favicon.svg')
    };
    ...
    const handleMediaSelect = (name: string, url: string | string[]) => {
        ...
        // Extract only the base name (filename) from the URL path
        const baseName = urlString ? urlString.split('/').pop() || urlString : '';
        setFormSettings(prev => ({ ...prev, [name]: baseName }));
    };
    ...
    const saveSettings = () => {
        ...
        router.post(route('appointment.settings.update'), {
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-3">
                                        ...
                                        <div className="border rounded-md p-4 flex items-center justify-center bg-muted/30 h-32">
                                                <img
                                                    src={formSettings.logo_dark ? getImagePath(formSettings.logo_dark) : defaultLogos.logo_dark}
                                                    alt={t('Dark Logo')}
                                                    className="max-h-full max-w-full object-contain"
                                                />
                                        </div>
                                        <MediaPicker
                                            value={formSettings.logo_dark || 'appointment_logo.png'}
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="border rounded-md p-4 flex items-center justify-center bg-muted/30 h-32">
                                                <img
                                                    src={formSettings.favicon ? getImagePath(formSettings.favicon) : defaultLogos.favicon}
                                                    alt={t('Favicon')}
                                                    className="h-16 w-16 object-contain"
                                                />
                                        </div>
                                        <MediaPicker
                                            value={formSettings.favicon || 'appointment_favicon.png'}
