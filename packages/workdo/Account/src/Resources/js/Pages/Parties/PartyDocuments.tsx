...
export default function PartyDocuments({ files, onChange, errors = {}, disabled = false, existingCount = 0 }: {
    files: File[];
    ...
    return <div className="space-y-2 border-t pt-4">
        ...
        <input ref={input} id={id} type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx,.zip" className="sr-only" disabled={disabled} onChange={(event) => {
            ...
            if (next.length + existingCount > 5) {
                setError(t('A customer or vendor can have up to 5 supporting documents.'));
                return;
            }
            if (next.some(file => file.size > 5 * 1024 * 1024 || !/\.(pdf|jpe?g|png|docx?|xlsx?|zip)$/i.test(file.name))) {
                setError(t('Select PDF, JPG, PNG, Word, Excel or ZIP files up to 5 MB each.'));
                return;
            }
            setError('');
            onChange(next);
        ...
        {files.map((file, index) => <div key={`${file.name}-${index}`}>
            <div className="flex min-w-0 items-center gap-2 border-b py-2 text-sm">
                ...
                <Button type="button" variant="ghost" size="icon" title={t('Remove file')} aria-label={t('Remove file')} disabled={disabled} onClick={() => onChange(files.filter((_, i) => i !== index))}>
                    <X className="h-4 w-4" />
                </Button>
            </div>
