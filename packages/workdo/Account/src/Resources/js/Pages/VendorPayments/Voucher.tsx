...
export default function Voucher() {
    ...
    const pageProps = usePage<VoucherProps>().props;
    ...
    const downloadPDF = async () => {
        ...
        const printContent = document.querySelector('.vendor-payment-voucher');
        if (printContent) {
            const opt = {
                margin: 0.25,
                filename: `vendor-payment-voucher-${payment.payment_number || payment.id}.pdf`,
    ...
    const totalDebitApplied = payment.debit_note_applications?.reduce((sum, application) => sum + Number(application.applied_amount || 0), 0) || 0;
    const totalApplied = totalAllocated + totalDebitApplied;
    ...
    return (
        <div className="min-h-screen bg-slate-100 py-6 print:bg-white print:py-0">
            <Head title={t('Vendor Payment Voucher')} />
            ...
            <div className="vendor-payment-voucher mx-auto max-w-4xl bg-white p-8 text-sm text-slate-900 shadow-sm print:shadow-none">
                <div className={`flex gap-6 border-b pb-6 ${headerLayoutClass}`}>
                    ...
                    <div className={`${badgeClass} min-w-52 rounded-lg p-4 text-white`} style={{ backgroundColor: color }}>
                        <div className="text-2xl font-bold">{t('PAYMENT VOUCHER')}</div>
                ...
                <div className="grid grid-cols-1 gap-6 py-6 md:grid-cols-2">
                    <div>
                        <div className="mb-2 font-semibold" style={{ color }}>{t('Paid To')}</div>
                        <div className="font-medium">{payment.vendor?.name || '-'}</div>
                        {payment.vendor?.email && <div>{payment.vendor.email}</div>}
                    </div>
                ...
                <div className="mb-6 flex justify-end">
                    <div className="w-full rounded-lg bg-slate-50 p-5 md:w-96">
                        <div className="flex items-center justify-between gap-4">
                            <span className="font-semibold uppercase tracking-wide text-slate-500">{t('Amount Paid')}</span>
                ...
                {payment.allocations && payment.allocations.length > 0 && (
                    <VoucherTable title={t('Purchase Invoice Allocations')} color={color}>
                        <thead>
                ...
                {payment.debit_note_applications && payment.debit_note_applications.length > 0 && (
                    <VoucherTable title={t('Debit Notes Applied')} color={color}>
                        <thead>
                            <tr className="text-white" style={{ backgroundColor: color }}>
                                <th className="px-3 py-2 text-left">{t('Debit Note Number')}</th>
                        ...
                        <tbody>
                            {payment.debit_note_applications.map((application) => (
                                <tr key={application.id} className="border-b">
                                    <td className="px-3 py-3 font-medium">{application.debit_note?.debit_note_number || '-'}</td>
                ...
                <div className="mt-6 flex justify-end">
                    <div className="w-72 space-y-2 rounded-lg bg-slate-50 p-4">
                        <TotalRow label={t('Amount Paid')} value={formatCurrency(payment.payment_amount, pageProps)} />
                        {totalDebitApplied > 0 && <TotalRow label={t('Debit Applied')} value={formatCurrency(totalDebitApplied, pageProps)} />}
        ...
        <div className="flex min-h-screen items-center justify-center bg-slate-100 p-6">
            ...
            <div className="max-w-lg rounded-lg bg-white p-8 text-center shadow-sm">
                ...
                <p className="mt-3 text-slate-600">
                    {t('Please create an active Invoice template before downloading vendor payment vouchers.')}
                </p>
...
function VoucherTable({ title, color, children }: { title: string; color: string; children: React.ReactNode }) {
    return (
        <div className="mb-8">
