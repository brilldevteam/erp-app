...
export default function NotesTab({ template, setDeleteConfig }: NotesTabProps) {
    ...
    const handleNoteSubmit = () => {
        if (editNoteId) {
            router.put(route('contract-template-notes.update', editNoteId), { note: noteText }, {
                onSuccess: () => {
            ...
            router.post(route('contract-template-notes.store', template.id), { note: noteText }, {
                onSuccess: () => {
                    setNoteDialogOpen(false);
                    setNoteText('');
                    router.reload();
    ...
    const handleCreateNoteSubmit = () => {
        router.post(route('contract-template-notes.store', template.id), { note: createNoteText }, {
            onSuccess: () => {
                setCreateNoteText('');
                router.reload();
            }
        ...
        <>
            <Card>
                ...
                <CardContent className="p-4">
                    {auth.user?.permissions?.includes('create-contract-template-notes') && (
                    ...
                    {(() => {
                        const filteredNotes = template.notes ? template.notes.filter((note: any) =>
                                                        ...
                                                        {(note.user_id === auth.user?.id || note.created_by === auth.user?.id) && (
                                                            <TooltipProvider>
                                                                <div className="flex gap-1">
                                                                    {auth.user?.permissions?.includes('edit-contract-template-notes') && (
                                                                    ...
                                                                    {auth.user?.permissions?.includes('delete-contract-template-notes') && setDeleteConfig && (
                                                                        <Tooltip delayDuration={0}>
                                                                            <TooltipTrigger asChild>
                                                                                <Button variant="ghost" size="sm" onClick={() => setDeleteConfig({
                                                                                    type: 'note',
                                                                                    id: note.id,
                                                                                    route: 'contract-template-notes.destroy',
