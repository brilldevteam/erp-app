...
export default function CommentsTab({ template, setDeleteConfig }: CommentsTabProps) {
    ...
    const handleCommentSubmit = () => {
        if (editCommentId) {
            router.put(route('contract-template-comments.update', editCommentId), { comment: commentText }, {
                onSuccess: () => {
            ...
            router.post(route('contract-template-comments.store', template.id), { comment: commentText }, {
                onSuccess: () => {
                    setCommentDialogOpen(false);
                    setCommentText('');
                    router.reload();
    ...
    const handleCreateCommentSubmit = () => {
        router.post(route('contract-template-comments.store', template.id), { comment: createCommentText }, {
            onSuccess: () => {
        ...
        <>
            <Card>
                ...
                <CardContent className="p-4">
                    {auth.user?.permissions?.includes('create-contract-template-comments') && (
                    ...
                    {(() => {
                        const filteredComments = template.comments ? template.comments.filter((comment: any) =>
                                                        ...
                                                        {(comment.user_id === auth.user?.id || comment.created_by === auth.user?.id) && (
                                                            <TooltipProvider>
                                                                <div className="flex gap-1">
                                                                    {auth.user?.permissions?.includes('edit-contract-template-comments') && (
                                                                    ...
                                                                    {auth.user?.permissions?.includes('delete-contract-template-comments') && setDeleteConfig && (
                                                                        <Tooltip delayDuration={0}>
                                                                            <TooltipTrigger asChild>
                                                                                <Button variant="ghost" size="sm" onClick={() => setDeleteConfig({
                                                                                    type: 'comment',
                                                                                    id: comment.id,
                                                                                    route: 'contract-template-comments.destroy',
