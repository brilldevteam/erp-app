...
export default function AttachmentsTab({ template, setDeleteConfig }: AttachmentsTabProps) {
    ...
    const paginatedAttachments = template.attachments ? {
        data: template.attachments.slice((attachmentPage - 1) * attachmentsPerPage, attachmentPage * attachmentsPerPage),
        total: template.attachments.length,
        last_page: Math.ceil(template.attachments.length / attachmentsPerPage)
    } : { data: [], total: 0, last_page: 1 };

    const handleAttachmentUpload = () => {
        if (selectedMedia.length > 0) {
            router.post(route('contract-template-attachments.store', template.id), {
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {auth.user?.permissions?.includes('create-contract-template-attachments') && (
                            <TooltipProvider>
                ...
                <CardContent className="p-0">
                    {paginatedAttachments.data.length > 0 ? (
                        <>
                                                    ...
                                                    <TooltipProvider>
                                                        <div className="flex gap-1">
                                                            {auth.user?.permissions?.includes('view-contract-templates') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button size="sm" variant="ghost" onClick={() => window.open(getImagePath(attachment.file_path, pageProps), '_blank')} className="h-8 w-8 p-0 text-green-600 hover:text-green-700">
                                                            ...
                                                            {auth.user?.permissions?.includes('view-contract-templates') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button size="sm" variant="ghost" onClick={() => downloadFile(getImagePath(attachment.file_path, pageProps))} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                                                            ...
                                                            {auth.user?.permissions?.includes('delete-contract-template-attachments') && setDeleteConfig && (attachment.uploaded_by === auth.user?.id || attachment.created_by === auth.user?.id) && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button size="sm" variant="ghost" onClick={() => setDeleteConfig({
                                                                            type: 'attachment',
                                                                            id: attachment.id,
                                                                            route: 'contract-template-attachments.destroy',
                                                    ...
                                                    <TooltipProvider>
                                                        <div className="flex gap-1">
                                                            {auth.user?.permissions?.includes('view-contract-templates') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button size="sm" variant="ghost" onClick={() => window.open(getImagePath(attachment.file_path, pageProps), '_blank')} className="h-6 w-6 p-0 text-green-600 hover:text-green-700">
                                                            ...
                                                            {auth.user?.permissions?.includes('view-contract-templates') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button size="sm" variant="ghost" onClick={() => downloadFile(getImagePath(attachment.file_path, pageProps))} className="h-6 w-6 p-0 text-blue-600 hover:text-blue-700">
                            ...
                            {template.attachments && template.attachments.length > attachmentsPerPage && (
                                <div className="px-4 py-3 border-t bg-gray-50/30">
                                    <div className="flex items-center justify-between">
                                        <div className="text-sm text-muted-foreground">
                                            {t('Showing')} <span className="font-medium text-foreground">{((attachmentPage - 1) * attachmentsPerPage) + 1}</span> {t('to')} <span className="font-medium text-foreground">{Math.min(attachmentPage * attachmentsPerPage, template.attachments.length)}</span> {t('of')} <span className="font-medium text-foreground">{template.attachments.length}</span> {t('results')}
                                        </div>
