                            ...
                            <div className="flex items-end gap-3">
                                ...
                                {auth.user?.permissions?.includes('print-profit-loss') && (
                                    <AccountingReportDownloadMenu
                                        onPdf={() => window.open(route('double-entry.profit-loss.print') + `?from_date=${fromDate}&to_date=${toDate}&download=pdf`, '_blank')}
                                        onExcel={() => { window.location.href = route('double-entry.profit-loss.excel') + `?from_date=${fromDate}&to_date=${toDate}`; }}
                                    />
                                )}
                            </div>
                        </div>
