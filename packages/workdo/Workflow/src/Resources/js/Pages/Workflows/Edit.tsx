...
export default function Edit() {
    ...
    const { workflow, modules } = usePage<Props>().props;
    useFlashMessages();
    ...
    useEffect(() => {
        // Initialize module fields on load
        const moduleData = modules.find(m => m.module === workflow.module && m.submodule === workflow.submodule);
        setSelectedModuleFields(moduleData?.fields || []);

        const moduleSubmodules = modules
            .filter(m => m.module === workflow.module)
            .map(m => m.submodule);
        setAvailableSubmodules(moduleSubmodules);

        // Load field values for existing conditions
        workflow.conditions.forEach((condition) => {
            if (condition.field) {
                const field = moduleData?.fields.find(f => f.name === condition.field);
                if (field) {
                    loadFieldValues(condition.field, field.model_name || 'default');
                }
            }
        });
    }, []);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('workflow.update', workflow.id));
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            pageTitle={t('Edit Workflow')}
        >
            <Head title={t('Edit Workflow')} />
