...
class AIService
{
    public function generateContent($fieldType, $settings, $context = [])
    {
        ...
        if (!$provider || !$model || !$apiKey) {
            throw new \Exception(__('AI configuration not found'));
        }
    ...
    private function callAI($provider, $model, $apiKey, $prompt, $settings)
    {
        switch ($provider) {
            ...
            default:
                throw new \Exception(__('Unsupported AI provider'));
        }
    }
    ...
    private function callAnthropic($model, $apiKey, $prompt, $settings)
    {
        ...
        throw new \Exception(__('Anthropic API error: ') . $response->body());
    }
    ...
    private function callGoogle($model, $apiKey, $prompt, $settings)
    {
        ...
        if ($response->successful()) {
            ...
            if (isset($data['error'])) {
                throw new \Exception(__('Google API error: ') . $data['error']['message']);
            }
            ...
            if (isset($data['candidates']) && !empty($data['candidates'])) {
                foreach ($data['candidates'] as $candidate) {
                    ...
                    if (isset($candidate['finishReason']) && $candidate['finishReason'] === 'SAFETY') {
                        throw new \Exception(__('Content was blocked by safety filters'));
                    }
                    ...
                    if (isset($candidate['content']['parts'][0]['text'])) {
                        ...
                        $text = __('Content truncated. Increase Maximum Length to 2000+ for full content.');
                    }
            ...
            if (empty($results)) {
                throw new \Exception(__('No content generated. Try increasing Maximum Length to 2000+ or simplify your prompt.'));
            }
        ...
        throw new \Exception(__('Google API HTTP error (' . $response->status() . '): ') . $errorBody);
    }
