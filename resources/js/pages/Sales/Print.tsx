    ...
    useEffect(() => {
        ...
        if (urlParams.get('download') === 'pdf' && fieldsLoaded) {
            ...
            const checkQRCode = () => {
                ...
                const qrImg = document.querySelector('[data-zatca-qr] img[src*="data:image"]');
                const errorDiv = document.querySelector('[data-zatca-qr] div');
                        ...
                        {pageButtons.length > 0 && (
                            <div className="mt-4 flex justify-end">
                                {pageButtons.map((button, index) => (
                                    <div key={`${button.id}-${index}`} data-zatca-qr className="hidden" id="zatca-qr-container">
                                        {button.component}
                                    </div>
                                ))}
                            </div>
