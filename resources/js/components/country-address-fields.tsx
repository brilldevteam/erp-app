        ...
        <div className="space-y-4">
            ...
            {code === 'QA' ? (
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {field('building_number', t('Building Number'), t('Enter 4-digit building number'), { required: requireAddressDetails, inputMode: 'numeric', pattern: '[0-9]{4}', maxLength: 4 })}
                    {field('street_name', t('Street Name'), t('Enter street name'), { required: requireAddressDetails })}
                    {field('district', t('District'), t('Enter district'), { required: requireAddressDetails })}
                    {field('city', t('City'), t('Enter city'), { required: requireAddressDetails })}
                    {field('zip_code', t('Postal Code'), t('Enter 5-digit postal code'), { required: requireAddressDetails, inputMode: 'numeric', pattern: '[0-9]{5}', maxLength: 5 })}
                    {field('secondary_number', t('Secondary Number'), t('Enter 4-digit secondary number'), { required: requireAddressDetails, inputMode: 'numeric', pattern: '[0-9]{4}', maxLength: 4 })}
                </div>
            ) : (code || address.country) ? (
                <>
                    {field('address_line_1', addressLineOneLabel || t('Address Line 1'), addressLineOnePlaceholder || t('Enter address'), { required: requireAddressDetails })}
                    {field('address_line_2', t('Address Line 2'), t('Apartment, suite, etc. (optional)'))}
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        {field('city', t('City'), t('Enter city'), { required: requireAddressDetails })}
                        {field('state', t('State / Province'), t('Enter state or province'), { required: requireAddressDetails })}
                        {field('zip_code', t('ZIP / Postal Code'), t('Enter ZIP or postal code'), { required: requireAddressDetails })}
                    </div>
                </>
            ) : null}
        </div>
